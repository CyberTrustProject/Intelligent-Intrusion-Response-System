...
class Topology:
    ...
    def add_nessus_report_information(self, nessus_file_path):
        ...
        for report_host in root.findall("Report/ReportHost"):
            ...
            for report_item in report_host.findall('ReportItem'):
                ...
                # Nessus compatibility isn't used.
                service = Service(svc_name, host_name_or_ip, port, protocol, 'cpe:/')
    ...
    def add_openvas_report_information(self, nessus_file_path):
        ...
        for report_host in results2[0].findall("result"):
            ...
            host_cpe = report_host.findall("host_cpe")[0].text
            if host_cpe:
                host.add_cpe(host_cpe)
            ...
            service_cpe = report_host.findall("service_cpe")[0].text

            service = Service(svc_name, host_name_or_ip, port, protocol, service_cpe)
            ...
            for service in host.services:
                ...
                if port >= 0 and protocol in ["tcp", "udp", "icmp"]:
                    ...
                    for vulnerability in service.vulnerabilities:
                        ...
                        if (vulnerability.cvss.attack_vector == "NETWORK") or (vulnerability.cvss.attack_vector == "ADJACENT_NETWORK"):
                            mulval_input_file.write(
                        ...
                        elif vulnerability.cvss.attack_vector == "LOCAL":
                            mulval_input_file.write(
            ...
            for flow_matrix_line in self.flow_matrix.lines:
                ...
                if mulval_destination_port and mulval_protocol:
                    if flow_matrix_line.source_element.type == FlowMatrixLineElement.IP and flow_matrix_line.destination_element.type == FlowMatrixLineElement.IP:
                        mulval_input_file.write(
                            "haclprimit('" + 
                            flow_matrix_line.source_element.resource + "','" + 
                            flow_matrix_line.destination_element.resource + "'," + 
                            mulval_destination_port + "," + mulval_protocol + ").\n"
                        )
...
class Host:
    ...
    def to_fiware_topology_xml_element(self):
        ...
        host_cpe = ET.SubElement(element, 'cpe')
        host_cpe.text = self.cpe
...
class Service:
    def __init__(self, name, ip, port, protocol, cpe):
        self._name = name
        self._ip = ip
        self._port = port
    ...
    def to_fiware_topology_xml_element(self):
        ...
        service_cpe = ET.SubElement(element, 'cpe')
        service_cpe.text = self.cpe
            ...
            with open(csv_file) as flow_matrix_csv_file:
                ...
                for flow_matrix_line in flow_matrix_csv:
                    if (len(flow_matrix_line) < 5) or flow_matrix_line[0] == "source":
                        ...
                        ########
                        # Add new services to the topology.
                        host = self.topology.get_host_by_ip(destination)
                        if host:
                            svc_name = 'flow-service ' + protocol + ' ' + destination_port
                            found = False
                            
                            for h in host.services:
                                if h.name == svc_name:
                                    found = True
                                    logging.warning("Found service {0} at host {1}.".format(svc_name, destination))
                                    break

                            if not found:
                                null_cve = 'CVE-0000-0000'
                                flow_service = Service(svc_name, destination, destination_port, protocol, 'cpe:/')
                                flow_service.add_vulnerability(null_cve, vulnerability_database)
                                logging.warning("[FLOW MATRIX] Added: {0} @ {1} -> {2} / {3} with {4}".format(svc_name, destination, destination_port, protocol, null_cve))
                                host.add_service(flow_service)
                        else:
                            logging.warning("[FLOW MATRIX] Host {0} was not found in the topology.".format(destination))
