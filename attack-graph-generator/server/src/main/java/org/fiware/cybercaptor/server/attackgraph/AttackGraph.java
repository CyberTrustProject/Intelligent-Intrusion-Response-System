...
public class AttackGraph implements Cloneable {
    ...
    private int[][] adjacencyMatrixEnchanced = null;
    /**
     * The global score of the attack graph
     */
    public double globalScore = 0;

    public ArrayList<VertexHostAssociation.VertexList> vertexToHostAssociations = null;
    ...
    public void deleteVertex(Integer id) throws Exception {
    ...
    public Element toDomElement() {
        ...
        for (Arc arc : arcs) {
            ...
            srcElement.setText(arc.source.id + "");
            arcElement.addContent(srcElement);
            Element dstElement = new Element("dst");
            dstElement.setText(arc.destination.id + "");
            arcElement.addContent(dstElement);

            Element prob = new Element("prob");
            prob.setText(arc.prob + "");
            arcElement.addContent(prob);
        }
        ...
        for (int key : vertices.keySet()) {
            ...
            Element idReducedElement = new Element("id_2");
            idReducedElement.setText(vertex.id2 + "");
            vertexElement.addContent(idReducedElement);
            ...
            Element init_Risk = new Element("init_risk");
            init_Risk.setText(vertex.init_Risk + "");
            vertexElement.addContent(init_Risk);
        }

        if (this.vertexToHostAssociations != null) {
            // Vertex to Host Association tables.
            Element verticesHostsElement = new Element("vertices_hosts");
            root.addContent(verticesHostsElement);

            for (VertexHostAssociation.VertexList l : this.vertexToHostAssociations) {
                if (!l.vertices.isEmpty()) {
                    Element listElement = new Element("list");
                    verticesHostsElement.addContent(listElement);

                    // Add the type of the list:
                    // IP|PORT|PROTOCOL|SERVICE    IP|PORT|PROTOCOL    IP|PORT         IP
                    // FULL_INFO                   PARTIAL_INFO        LIMITED_INFO    IP_ONLY
                    Element typeElement = new Element("type");
                    typeElement.addContent(l.type.name());
                    listElement.addContent(typeElement);

                    if (l.type == VertexHostAssociation.VertexListType.FULL_INFO) {
                        Element ipElement = new Element("ip");
                        ipElement.addContent(l.ip);
                        listElement.addContent(ipElement);

                        Element portElement = new Element("port");
                        portElement.addContent(String.valueOf(l.port));
                        listElement.addContent(portElement);

                        Element protocolElement = new Element("protocol");
                        protocolElement.addContent(l.protocol);
                        listElement.addContent(protocolElement);

                        Element serviceElement = new Element("service");
                        serviceElement.addContent(l.service.getName());
                        listElement.addContent(serviceElement);

                        Element verticesListElement = new Element("vertices");
                        listElement.addContent(verticesListElement);
                        for (Vertex v : l.vertices) {
                            Element vertexIdElement = new Element("id");
                            vertexIdElement.addContent(String.valueOf(v.id));
                            verticesListElement.addContent(vertexIdElement);
                        }
    ...
    public AttackGraph clone() throws CloneNotSupportedException {
        AttackGraph copy = (AttackGraph) super.clone();

        // Copy the vertices
        copy.vertices = new HashMap<Integer, Vertex>();
        for (Integer i : this.vertices.keySet()) {
            copy.vertices.put(i, this.vertices.get(i).clone());
        }

        copy.arcs = new ArrayList<Arc>();

        // Change all the vertices references in the arcs
        for (int i = 0; i < this.arcs.size(); i++) {
            Vertex destination = copy.vertices.get(this.arcs.get(i).destination.id);
            Vertex source = copy.vertices.get(this.arcs.get(i).source.id);
            Arc arc = new Arc(source, destination);
            copy.arcs.add(arc);
        }

        // Change all the references in the vertices
        for (Integer i : copy.vertices.keySet()) {
            Vertex copieVertex = copy.vertices.get(i);
            copieVertex.computeParentsAndChildren(copy);
        }

        // Copy the vertex-host associations.
        copy.vertexToHostAssociations = new ArrayList<VertexHostAssociation.VertexList>();
        for (VertexHostAssociation.VertexList l : this.vertexToHostAssociations) {
            copy.vertexToHostAssociations.add(l.clone());
        }
    ...
    public void enrichVertexInfo(InformationSystem informationSystem) throws Exception {
        if (this.vertexToHostAssociations == null) {
            // Can't do anything as the ArrayList hasn't been initialized.
            // If called correctly it should never throw that though.
            throw new IllegalStateException();
        }

        for (int i = 0; i < this.vertexToHostAssociations.size(); i++) {
            VertexHostAssociation.VertexList l = this.vertexToHostAssociations.get(i);

            // FULL_INFO = IP, Port, Protocol, Service
            // PARTIAL_INFO = IP, Port, Protocol (will have the exact same contents)
            if (l.type == VertexHostAssociation.VertexListType.FULL_INFO) {
                for (Vertex v : l.vertices) {
                    if (v.concernedMachine == null) {
                        v.concernedMachine = informationSystem.existingMachineByNameOrIPAddress(l.ip);
                    }

                    if (v.concernedService == null) {
                        v.concernedService = l.service;
                    }

                    if (v.concernedService != null) {
                        if ((v.fact != null) && (v.fact.type == FactType.DATALOG_FACT) && (v.fact.datalogCommand != null)) {
                            if (v.fact.datalogCommand.command.equals("vulExists") && (v.fact.datalogCommand.params.length >= 1)) {
                                String cve = v.fact.datalogCommand.params[1];
                                try {
                                    v.relatedVulnerabilibty = v.concernedService.getNewOrExistingVulnerability(cve);
                                } catch (Exception e) {
                                    e.printStackTrace();
                                }
                            }
            ...
            if (this.vertexToHostAssociations.size() == (i + 1)) {
                return;
            }
        }
    }

    public ArrayList<Vertex> findRootVertices() {

        ArrayList<Vertex> roots = new ArrayList<Vertex>();
        for (int i = 1; i <= this.vertices.size(); i++) {

            if (this.vertices.get(i).children.isEmpty() || this.vertices.get(i).children == null) {
    ...
    public Arc getArcByVertices(Vertex source1, Vertex destination1) { 

        for (int i = 0; i < arcs.size(); i++) {

            if ((arcs.get(i).source.equals(source1) && arcs.get(i).destination.equals(destination1)) || (arcs.get(i).source.equals(destination1) && arcs.get(i).destination.equals(source1))) {
    ...
    public ArrayList<Arc> getParentsArcs(Vertex arcDestination) {

        ArrayList<Arc> parentsArcs = new ArrayList<Arc>();

        for (int p = 0; p < this.arcs.size(); p++) {

            if (this.arcs.get(p).destination == arcDestination) {

                parentsArcs.add(arcs.get(p));
    ...
    private void enchancedAM() {

        int numberOfVertices = this.getNumberOfVertices();

        System.out.println("SIZE:  " + numberOfVertices);

        this.adjacencyMatrixEnchanced = new int[numberOfVertices][numberOfVertices];

        for (int i = 0; i < this.arcs.size(); i++) {

            Arc arc = this.arcs.get(i);
            this.adjacencyMatrixEnchanced[arc.source.id - 1][arc.destination.id - 1] = i + 1;
        }


    }

    public Arc getArcByVertices2(Vertex source, Vertex destination) throws Exception {

        int numberOfVertices = this.getNumberOfVertices();

        if (source.id > numberOfVertices || destination.id > numberOfVertices) {

            System.out.println(" [ERROR] Source or Destination not in AG Vertices");
            return null;
        }

        int arcID = this.adjacencyMatrixEnchanced[source.id - 1][destination.id - 1];
        if (arcID != 0) {

            return this.arcs.get(arcID - 1);
        } else {

            System.out.println(" [ERROR] Arc does not exist");
            return null;
        }
