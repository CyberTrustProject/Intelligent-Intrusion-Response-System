        ...
        for (Host host : topology.getHosts()) {
            ...
            for (Service service : informationSystemHost.getServices().values()) {
                ...
                for (String cve : service.getVulnerabilities().keySet()) {
                    ...
                    if (vulnerability.cvss_3 == null) {
                        if (vulnerability.cvss_2 != null && vulnerability.cvss_2.getScore() >= 6.6) {
                            fichier.println("cvss('" + vulnerability.cve + "',h).");
                        } else if (vulnerability.cvss_2 != null && vulnerability.cvss_2.getScore() >= 3.3) {
                            fichier.println("cvss('" + vulnerability.cve + "',m).");
                        } else if (vulnerability.cvss_2 != null && vulnerability.cvss_2.getScore() > 0) {
                            fichier.println("cvss('" + vulnerability.cve + "',l).");
                        } else {
                            fichier.println("cvss('" + vulnerability.cve + "',m).");
                        }
                    } else {
                        if (vulnerability.cvss_3 != null && vulnerability.cvss_3.getScore() >= 6.6) {
                            fichier.println("cvss('" + vulnerability.cve + "',h).");
                        } else if (vulnerability.cvss_3 != null && vulnerability.cvss_3.getScore() >= 3.3) {
                            fichier.println("cvss('" + vulnerability.cve + "',m).");
                        } else if (vulnerability.cvss_3 != null && vulnerability.cvss_3.getScore() > 0) {
                            fichier.println("cvss('" + vulnerability.cve + "',l).");
                        } else {
                            fichier.println("cvss('" + vulnerability.cve + "',m).");
                        }
            ...
            for (FlowMatrixLine flowMatrixLine : this.flowMatrix.getFlowMatrixLines()) {
                ...
                if (!mulvalDestinationPort.isEmpty() && !mulvalProtocol.isEmpty()) {
                    ...
                    if (sourceElement.getType().equals(FlowMatrixElement.FlowMatrixElementType.IP) &&
                        ...
                        System.out.println("sourceElement.getType " + sourceElement.getType());
                        System.out.println("destinationElement.getType " + destinationElement.getType());
