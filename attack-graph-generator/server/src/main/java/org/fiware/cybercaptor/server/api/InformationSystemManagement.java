...
public class InformationSystemManagement {
     ...
     */
    public static AttackGraph executeMulval(File mulvalInputFile, File mulvalOutputFile, String attackGraphXMLOutput) throws Exception {
        ...
        RestApplication.print_message(Level.INFO, "Launching MulVAL.");
        ProcessBuilder processBuilder = new ProcessBuilder(mulvalPath + "/utils/graph_gen.sh", mulvalInputFile.getAbsolutePath(), "-l");

        // MulVAL argument to set the name of the XML attack graph output.
        processBuilder.command().add("-o");
        processBuilder.command().add(outputFolderPath + attackGraphXMLOutput);

        // MulVAL argument to only use the custom ruleset.
        ...
        if (!mulvalOutputFile.exists()) {
            RestApplication.print_message(Level.SEVERE, "Empty attack graph.");
            return null;
        }
    ...
    public static void generateXmlTopologyFromDiskFiles() throws Exception{

        //Load python script properties
        String pythonPath = ProjectProperties.getProperty("python-path");
        String dataExtractionModuleFolder = ProjectProperties.getProperty("mulval-input-script-folder");
        String dataExtractionModulePath = dataExtractionModuleFolder + "main.py";
        ...
        // Prepare to execute the CyberCAPTOR Data Extraction module to generate the FIWARE topology XML file.
        ProcessBuilder processBuilder = new ProcessBuilder(pythonPath, dataExtractionModulePath,
        ...
        //--openvas-scan    --vulnerability-scan

        // Set the working directory.
        processBuilder.directory(new File(dataExtractionModuleFolder));
        ...
        File topologyXML = new File(topologyPath);

        if (!topologyXML.exists()) {
            Logger.getAnonymousLogger().log(Level.WARNING, "A problem happened in the generation of mulval inputs");
            throw new IllegalArgumentException("Couldn't generate the topology XML file from data on disk");
        }
    }
